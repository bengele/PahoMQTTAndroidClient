...
class AlarmPingSender implements MqttPingSender {
    ...
    public void start() {
        ...
        NLogger.d(TAG, "Register alarmreceiver to MqttService" + action);
    ...
    public void stop() {
        ...
        NLogger.d(TAG, "Unregister alarmreceiver to MqttService" + comms.getClient().getClientId());
        if (hasStarted) {
    ...
    public void schedule(long delayInMilliseconds) {
        long nextAlarmInMilliseconds = System.currentTimeMillis()
                + delayInMilliseconds;
        NLogger.d(TAG, "Schedule next alarm at " + nextAlarmInMilliseconds);
...
//            NLogger.d(TAG, "Alarm scheule using setExactAndAllowWhileIdle, next: " + delayInMilliseconds);
            //			alarmManager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, nextAlarmInMilliseconds, pendingIntent);
            NLogger.d(TAG, "Alarm scheule using setAlarmClock, next: " + delayInMilliseconds);
            alarmManager.setAlarmClock(new AlarmManager.AlarmClockInfo(nextAlarmInMilliseconds, pendingIntent), pendingIntent);
        } else if (Build.VERSION.SDK_INT >= 19) {
            NLogger.d(TAG, "Alarm scheule using setExact, delay: " + delayInMilliseconds);
    ...
    class AlarmReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            NLogger.d(TAG, "Sending Ping at:" + System.currentTimeMillis());
            ...
            IMqttToken token = comms.checkForActivity(new IMqttActionListener() {
                ...
                public void onSuccess(IMqttToken asyncActionToken) {
                    NLogger.d(TAG, "Success. Release lock(" + wakeLockTag + "):"
                ...
                public void onFailure(IMqttToken asyncActionToken,
                                      Throwable exception) {
                    NLogger.d(TAG, "Failure. Release lock(" + wakeLockTag + "):"
                            + System.currentTimeMillis());
                    //Release wakelock when it is done.
                    wakelock.release();
                }
